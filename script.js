...
function getColVal(item, name) {
  ...
  for (var i = 0; i < cols.length; i++) {
    if (cols[i].getAttribute('name') === name) return cols[i].textContent.trim();
  }
  return '';
}
    ...
    .then(function(text) {
      ...
      xml.querySelectorAll('item').forEach(function(item) {
        result.push({
          ...
          region: getColVal(item,'supt_regin') || '전국',
...
function renderSlider() {
  ...
  var pageItems = sliderItems.slice(sliderPage * sliderPerPage, (sliderPage+1) * sliderPerPage);
  ...
  list.innerHTML = '<div>'
    ...
    + '<button onclick="slideNext()" style="background:' + (sliderPage>=totalPages-1?'#e2e8f0':'#cbd5e1') + '; border:none; border-radius:50%; width:36px; height:36px; font-size:20px; cursor:pointer; flex-shrink:0;">›</button>'
    + '</div>'
    + '</div>';
}
...
function submitWaitlist(e) {
  ...
  if (!email || !interest || !sido) { alert('필수 항목을 입력해 주세요.'); return; }
  ...
  fetch('https://script.google.com/macros/s/AKfycbxWbDq8ax0CaVBfQqwt5GlvgKLQf8yVZCdAhtTR3MVgydKKh3QScR1ZLDTR2-s8cuxw/exec', {
    ...
    body: JSON.stringify({ bizNum: biz, email: email, phone: phone, sido: sido, interest: interest })
  })
  .then(function() {
  ...
  .catch(function() {
    ...
    btn.textContent = '무료로 맞춤 지원금 사전 등록하기'; btn.disabled = false;
  });
}
